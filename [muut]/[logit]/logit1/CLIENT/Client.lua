		...
		if IsEntityDead(PlayerPedId()) then
			...
			elseif (Killer == nil) then
				...
				elseif IsBomb(DeathCauseHash) then
					DeathReason = 'räjäytti'
				elseif IsVeh(DeathCauseHash) then
					DeathReason = 'ajoi yli'
			...
			if DeathReason == 'itsemurha' or DeathReason == 'sairaskohtaus' then
				TriggerServerEvent('DiscordBot:playerDied', '`' .. GetPlayerName(PlayerId()) .. '` ' .. DeathReason .. ' ', Weapon)
			else
				TriggerServerEvent('DiscordBot:playerDied', '`' .. GetPlayerName(Killer) .. '` ' .. DeathReason .. ' **Pelaajan:** `' .. GetPlayerName(PlayerId()) .. '` ', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
