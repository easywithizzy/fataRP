...
function TchatAddMessage (channel, message)
  ...
  MySQL.Async.insert(Query, Parameters, function (id)
    MySQL.Async.fetchAll(Query2, { ['@id'] = id }, function (reponse)
      TriggerClientEvent('gcPhone:sendMessage:tchat_receive', -1, reponse[1])
    end)
  end)
end


RegisterServerEvent('gcPhone:sendMessage:tchat_channel')
AddEventHandler('gcPhone:sendMessage:tchat_channel', function(channel)
  local sourcePlayer = tonumber(source)
  TchatGetMessageChannel(channel, function (messages)
    TriggerClientEvent('gcPhone:sendMessage:tchat_channel', sourcePlayer, channel, messages)
  end)
end)

RegisterServerEvent('gcPhone:sendMessage:tchat_addMessage')
AddEventHandler('gcPhone:sendMessage:tchat_addMessage', function(channel, message)
