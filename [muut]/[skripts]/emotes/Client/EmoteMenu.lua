...
function AddEmoteMenu(menu)
    ...
    if not Config.SqlKeybinding then
      ...
      keyinfo =  NativeUI.CreateItem(Config.Languages[lang]['keybinds'], Config.Languages[lang]['keybindsinfo'].." ~b~/emotebind ~b~[~y~~b~num4-9~w~~b~] ~b~[~g~~b~emotename~w~~b~]")
      submenu:AddItem(keyinfo)
    end

    for a,b in pairsByKeys(DP.Emotes) do
      x,y,z = table.unpack(b)
      emoteitem = NativeUI.CreateItem(z, "~b~/e ("..a..")")
    ...
    for a,b in pairsByKeys(DP.Dances) do
      x,y,z = table.unpack(b)
      danceitem = NativeUI.CreateItem(z, "~b~/e ("..a..")")
    ...
    if Config.SharedEmotesEnabled then
      for a,b in pairsByKeys(DP.Shared) do
        ...
        if otheremotename == nil then
          shareitem = NativeUI.CreateItem(z, "~b~/nearby (~g~"..a.."~w~)")
        else 
          shareitem = NativeUI.CreateItem(z, "~b~/nearby (~g~"..a.."~w~~b~) "..Config.Languages[lang]['makenearby'].." (~y~"..otheremotename.."~w~~b~)")
        end
    ...
    for a,b in pairsByKeys(DP.PropEmotes) do
      x,y,z = table.unpack(b)
      propitem = NativeUI.CreateItem(z, "~b~/e ("..a..")")
...
function AddWalkMenu(menu)
    ...
    WalkInjured = NativeUI.CreateItem("~b~Injured", "")
...
function AddInfoMenu(menu)
    if not UpdateAvailable then
      infomenu = _menuPool:AddSubMenu(menu, Config.Languages[lang]['infoupdate'], "~b~(1.7.3)", "", Menuthing, Menuthing)
    else
    ...
    u170 = NativeUI.CreateItem("~b~1.7.0", "~b~Added /emotebind [key] [emote]!")
    u165 = NativeUI.CreateItem("~b~1.6.5", "~b~Updated camera/phone/pee/beg, added makeitrain/dance(glowstick/horse).")
    u160 = NativeUI.CreateItem("~b~1.6.0", "~b~Added shared emotes /nearby, or in menu, also fixed some emotes!")
    u151 = NativeUI.CreateItem("~b~1.5.1", "~b~Added /walk and /walks, for walking styles without menu")
    u150 = NativeUI.CreateItem("~b~1.5.0", "~b~Added Facial Expressions menu (if enabled by server owner)")
