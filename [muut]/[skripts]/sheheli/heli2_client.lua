...
-- Määritykset
local fov_max = 80.0
local fov_min = 10.0 -- suurin suurennus taso (pienempi fov on enemmän suurennusta)
local zoomspeed = 2.0 -- kameran suurennus nopeus
local speed_lr = 3.0 -- speed by which the camera pans left-right 
local speed_ud = 3.0 -- speed by which the camera pans up-down
local toggle_helicam = 51 -- näppäin joka aktivoi kameran. Oletus: INPUT_CONTEXT (E)
local toggle_vision = 25 -- näppäin joka aktivoi lämpö kameran. Oletus: INPUT_AIM (oikea hiiren painike)
local toggle_rappel = 154 -- control id to rappel out of the heli. Oletus: INPUT_DUCK (X)
local toggle_spotlight = 183 -- näppäin joka aktivoi etsintävalon edessä Oletus: INPUT_PhoneCameraGrid (G)
local toggle_lock_on = 22 -- näppäin joka lukitsee kameran ajoneuvoon. Oletus: INPUT_SPRINT (spacebar)

-- Skripti alkaa tästä
local helicam = false
local as332_hash = GetHashKey("as332")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- [0] = Normaali, [1] = Yö tila, [2] = Lämpökamera
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInAs332() then
			local lPed = GetPlayerPed(-1)
			local heli = GetVehiclePedIsIn(lPed)
			
			if IsHeliHighEnough(heli) then
				if IsControlJustPressed(0, toggle_helicam) then -- Aktivoi kameran
...
function CheckInputRotation(cam, zoomvalue)
	local rightaxi:paysX = GetDisabledControlNormal(0, 220)
	local rightaxi:paysY = GetDisabledControlNormal(0, 221)
	local rotation = GetCamRot(cam, 2)
	if rightaxi:paysX ~= 0.0 or rightaxi:paysY ~= 0.0 then
		new_z = rotation.z + rightaxi:paysX*-1.0*(speed_ud)*(zoomvalue+0.1)
		new_x = math.max(math.min(20.0, rotation.x + rightaxi:paysY*-1.0*(speed_lr)*(zoomvalue+0.1)), -89.5) -- Clamping at top (cant see top of heli) and at bottom (doesn't glitch out in -90deg)
