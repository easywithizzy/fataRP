...
RegisterNetEvent("radiallukko")
AddEventHandler("radiallukko", function()
            -- Init player infos
            local ply = GetPlayerPed(-1)
            local pCoords = GetEntityCoords(ply, true)
            local px, py, pz = table.unpack(GetEntityCoords(ply, true))
...
function Notify(text, duration)
	if(Config.notification)then
		if(Config.notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_MP_DETONATEPHONE", "CHAR_MP_DETONATEPHONE", true, 1, "Keskuslukitus", " ", duration)
