	...
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) then 
		if (GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1)) then
			...
			if vehicles[StateIndex][2] then
				exports['mythic_notify']:DoLongHudText('success', 'Moottori on Käynnistetty')
			else
				exports['mythic_notify']:DoLongHudText('error', 'Moottori on Sammutettu')
			end
		end 
    end 
end)
			...
			if GetSeatPedIsTryingToEnter(GetPlayerPed(-1)) == -1 then
				for i, vehicle in ipairs(vehicles) do
					if vehicle[1] == GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) and not vehicle[2] then
						Citizen.Wait(1000)
						vehicle[2] = true
						notify('~g~Moottori on Käynnistetty!')
					end
				end
			end
		end
