  ...
  while true do
    ...
    if GetPedInVehicleSeat(vehicle, -1) == playerPed and IsPedInAnyVehicle(playerPed, false) then
      ...
      if IsControlJustReleased(0,246) and IsControlPressed(0,131) then
        maxSpeed = GetVehicleHandlingFloat(vehicle,"CHandlingData","fInitialDriveMaxFlatVel")
        SetEntityMaxSpeed(vehicle, maxSpeed)
        showHelpNotification("~r~Nopeusrajotin sammutettu!")
      ...
      elseif IsControlJustReleased(0,246) then
        ...
        if useMph then
          ...
          showHelpNotification("~g~Nopeusrajoitettu nopeuteen ~b~"..cruise.." ~g~km/h. ~INPUT_VEH_SUB_ASCEND~ + ~INPUT_MP_TEXT_CHAT_TEAM~ sammuutaaksesi.")
        end
      end
    else
