...
AddEventHandler("tac:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("tac-kr-advaced-shops-1:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("tac-kr-advaced-shops-1:openmodimenu")
AddEventHandler("tac-kr-advaced-shops-1:openmodimenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("tac-kr-advaced-shops-1:jailPlayer")
AddEventHandler("tac-kr-advaced-shops-1:jailPlayer", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("tac-kr-advaced-shops-1:unJailPlayer")
AddEventHandler("tac-kr-advaced-shops-1:unJailPlayer", function()
	jailTime = 0
...
function UnJail()
	...
	ESX.ShowNotification("Sinut vapautetaan, pysy rauhallisena ulkona! Onnea!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Sinulla on " .. jailTime .. " Minuuttia jäljellä vankilassa!")

			TriggerServerEvent("tac-kr-advaced-shops-1:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("tac-kr-advaced-shops-1:updateJailTime", 0)
			end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Avataksesi oven", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			exports['mythic_notify']:DoLongHudText('error', 'Peruutettu!')
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Pakataan... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Lopeta pakkaaminen", 0.4)
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("tac-kr-advaced-shops-1:prisonWorkReward")
				end
			end
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'bottom-right',
			elements = {
				{ label = "Vangitse lähin kaupunkilainen", value = "jail_closest_player" },
				{ label = "Poista tuomio kaupunkilaiselta", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Vankila aika (minuuteissa)"
          		},
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Ajan on oltava minuutteina!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Sinun pitää laittaa jotain tänne!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("tac-kr-advaced-shops-1:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("tac-kr-advaced-shops-1:retrieveJailedPlayers", function(playerArray)

				if #playerArray == 0 then
					ESX.ShowNotification("Sinun vankila on tyhjä!")
					return
				end
				...
				function(data2, menu2)
					...
					TriggerServerEvent("tac-kr-advaced-shops-1:unJailPlayer", action)
