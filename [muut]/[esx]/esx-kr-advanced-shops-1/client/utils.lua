RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
        ...
        exports['mythic_notify']:DoLongHudText('error', 'Et ole poliisi!')
	end
end)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" or "sheriff" then
					exports['mythic_notify']:DoLongHudText('error', 'Sinulla ei ole avainta mennäksesi tänne!')
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    SetBlipSprite (blip, 238) -- 238 Is orginal
