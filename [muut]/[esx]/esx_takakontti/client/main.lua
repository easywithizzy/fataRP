...
Citizen.CreateThread(function()
	while true do
		if (trunkIsOpen and currentPlate ~= nil and not IsDistanceOk()) or (trunkIsOpen and IsPedInAnyVehicle(GetPlayerPed(-1))) or (trunkIsOpen and IsDoorLocked()) then
			TriggerEvent('tac_vehicletrunk:onQuit')
			Citizen.Wait(500)
		end
		Citizen.Wait(100)
	end
...
AddEventHandler('tac_vehicletrunk:onTrunkOpen', function()
	SetVehicleDoorOpen(currentVehicle,5,0,0)
end)

AddEventHandler('tac_vehicletrunk:onTrunkClose', function()
	SetVehicleDoorShut(currentVehicle,5,0)
end)

RegisterNetEvent('tac_vehicletrunk:addAmmo', function(weapon, ammo)
	AddAmmoToPed(GetPlayerPed(-1), weapon , ammo)
end)

AddEventHandler('tac_vehicletrunk:onQuit', function()
	if not trunkIsOpen then
		return
	end
	trunkIsOpen = false
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('tac_vehicletrunk:release', currentPlate, json.encode(currentContent), currentExists)
	ESX.ShowNotification('Peräkontin sisältö ~g~tallennettu~w~.')
	TriggerEvent('tac_vehicletrunk:onTrunkClose')
end)
	...
	for i = 1, #currentContent, 1 do
		if currentContent[i].name == name then
			if currentContent[i].count + count > limit then
				...
				TriggerServerEvent('tac_vehicletrunk:removeItem', name, count)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if count == nil or count < 1 or (take and count > itemCount) or (not take and count > itemLimit) or (not take and count > itemCount) then
				ESX.ShowNotification("~r~>Virheellinen ~w~lukumäärä~r~.")
			elseif take then
				TakeItem(item, count)
			else
...
function AddItemsMenu()
	if #currentContent >= Config.MaxItems then
		ESX.ShowNotification('~w~Peräkontin ~y~tilavuus ~r~ei~w~ riitä~r~!')
		OpenTrunkMenu()
		return
	end
...
function AddCashMenu()
	...
	'dialog', GetCurrentResourceName(), 'failed_at_coding_dialog3',
	{
		...
		if cash == nil then
			ESX.ShowNotification("Tapahtui ~r~virhe~w~.")
		else
			menu.close()
			ESX.TriggerServerCallback('tac_vehicletrunk:addCash', function(sum)
				local sum = sum
				if sum ~= -1 then
					local found = false
					for i = 1, #currentContent, 1 do
...
function AddDirtyMenu()
	...
	'dialog', GetCurrentResourceName(), 'failed_at_coding_dialog3',
	{
		...
		if cash == nil then
			...
			menu.close()
			ESX.TriggerServerCallback('tac_vehicletrunk:addDirty', function(sum)
				if sum ~= -1 then
					local found = false
					for i = 1, #currentContent, 1 do
			...
			for i = 1, #currentContent, 1 do
				if currentContent[i].type == 'dirty' then
					if currentContent[i].count > cash then
						TriggerServerEvent('tac_vehicletrunk:giveDirty', cash)
	...
	'dialog', GetCurrentResourceName(), 'failed_at_coding_dialog5',
	{
		...
		if cash ~= nil and cash >= 1 then
			...
			for i = 1, #currentContent, 1 do
				if currentContent[i].type == 'cash' then
					if currentContent[i].count > cash then
						TriggerServerEvent('tac_vehicletrunk:giveCash', cash)
						currentContent[i].count = currentContent[i].count - cash
					elseif currentContent[i].count == cash then
						TriggerServerEvent('tac_vehicletrunk:giveCash', cash)
						table.remove(currentContent, i)
...
function TakeWeapon(name, ammo)
	...
	if not found then
		...
		TriggerServerEvent('tac_vehicletrunk:giveWeapon', name, ammo)
	end
end
...
function TakeItem(name, count)
	...
	if not found then
		...
		TriggerServerEvent('tac_vehicletrunk:giveItem', name, count, xPlayer)
	end
end

function AddWeapon(name, ammo)
	local found = false
	TriggerServerEvent('tac_vehicletrunk:removeWeapon', name)
...
function AddWeaponsMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      menu.close()
	  if #currentContent >= Config.MaxItems then
		ESX.ShowNotification('~y~Peräkontin ~w~tilavuus ~r~ei ~w~riitä.')
		Wait(500)
		OpenTrunkMenu()
	  else
		ESX.UI.Menu.Open(
...
function InitTrunkMenu()
	...
	ESX.TriggerServerCallback('tac_vehicletrunk:openTrunk', function(trunk)
		if not trunk.error then
			TriggerEvent('tac_vehicletrunk:onTrunkOpen')
