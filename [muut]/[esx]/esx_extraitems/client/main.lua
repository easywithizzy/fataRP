...
AddEventHandler('tac_phone:loaded', function(phoneNumber, contacts)
	...
	if count > 0 then
		...
		TriggerEvent('tac_phone:removeSpecialContact', specialContact.number)
	end
end)
...
function CheckInputRotation(cam, zoomvalue)
	...
	if rightaxi:paysX ~= 0.0 or rightaxi:paysY ~= 0.0 then
		...
		new_x = math.max(math.min(20.0, rotation.x + rightaxi:paysY*-1.0*(speed_lr)*(zoomvalue+0.1)), -89.5)
