...
function registerJobChangeListener()
    ...
    AddEventHandler('tac:setJob', handleJobChange)
end
...
RegisterNetEvent("tac_taxi:paymeter:updatePassenger")
AddEventHandler("tac_taxi:paymeter:updatePassenger", function(attributes)
  meterAttrs = attributes
  meterOwner = false
  updateMeter()
end)

RegisterNetEvent("tac_taxi:paymeter:updateLocation")
AddEventHandler("tac_taxi:paymeter:updateLocation", function()
  lastLocation = GetEntityCoords(GetPlayerPed(-1))
end)



RegisterNetEvent("tac_taxi:paymeter:resetMeter")
AddEventHandler("tac_taxi:paymeter:resetMeter", function()
  resetMeter()
  updateMeter()
end)
...
function updatePassengerMeters()
  ...
  for index,player in ipairs(players) do
    ...
    if not has_value(playersInVehicle, playerId) then
      TriggerServerEvent('tac_taxi:paymeter:updatePassengerMeters', playerId, meterAttrs)
...
function resetMeter()
  ...
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('tac_taxi:paymeter:resetPassengerMeters', player)
  end
end

function updatePassengerLocations()
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('tac_taxi:paymeter:updatePassengerLocation', player)
  end
end
