...
AddEventHandler('tac-br-rob-humane:toofar', function(robb)
	...
	for i=1, #xPlayers, 1 do
		 ...
		 if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' then
			TriggerClientEvent('tac:showNotification', xPlayers[i], _U('ryöstö_peruutettu_kohteessa') .. bank:transfers[robb].nameofbank:transfer)
	...
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('tac:showNotification', source, _U('ryöstö_on_peruutettu') .. bank:transfers[robb].nameofbank:transfer)
	end
end)
...
AddEventHandler('tac-br-rob-humane:rob', function(robb)
	...
	if bank:transfers[robb] then
		...
		if (os.time() - bank:transfer.lastrobbed) < 43200 and bank:transfer.lastrobbed ~= 0 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' then
							...
							TriggerClientEvent('tac-br-rob-humane:setblip', xPlayers[i], bank:transfers[robb].position)
					end
				end
