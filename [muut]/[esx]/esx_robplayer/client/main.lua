...
function IsAbleToSteal(targetSID, err)
    ESX.TriggerServerCallback('tac_robplayer:getValue', function(result)
        local result = result
...
function OpenStealMenu(target, target_id)
    ...
    ESX.TriggerServerCallback('tac_robplayer:getOtherPlayerData', function(data)

        local elements = {}

        if Config.EnableCash then
            table.insert(elements, {
                label      = 'Käteinen:' .. ' ' .. data.money .. ' €',
                value      = 'money',
                type       = 'item_money',
                amount     = data.money,
            })
        ...
        if Config.EnableBlackMoney then
            ...
            table.insert(elements, {
              label          = 'Likainen Raha:' .. ' ' .. blackMoney .. ' €',
              value          = 'black_money',
              type           = 'item_account',
              amount         = blackMoney,
            })
        ...
        if Config.EnableInventory then
            table.insert(elements, {label = '--- ' .. 'Esineet' .. ' ---', value = nil})
		...
		if Config.EnableWeapon then
            ...
            for i=1, #data.weapons, 1 do
				local weaponName = data.weapons[i].name
				local weaponHash = GetHashKey(weaponName)
				--[[
				print(weaponName)
				print(weaponHash)
				print(data.weapons[i].ammo)]]--
				if data.weapons[i].name ~= nil then
					--[[
					print(data.weapons[i].label)
					print(data.weapons[i].name) ]]--
					table.insert(elements, {
                    ...
                    function(data2, menu2)
                        ...
                        if data2.current.action == 'steal' then
                            ...
                            if itemType == 'item_standard' then
                                ...
                                ESX.UI.Menu.Open(
                                    ...
                                    function(data3, menu3)
                                        ...
                                        TriggerServerEvent('tac_robplayer:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, quantity)
							...
							elseif itemType == 'weapons' then
								...
								print(itemName)
								print(amount)
							    TriggerServerEvent('tac_robplayer:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, amount)
								print("kössittiin")
                                OpenStealMenu(target)
                              
                            else
								print("viimiseen meni")
                                TriggerServerEvent('tac_robplayer:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, amount)
                                OpenStealMenu(target)
                            end
