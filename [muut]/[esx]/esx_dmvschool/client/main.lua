...
function StartTheoryTest()
	...
	TriggerServerEvent('tac_dmv:successschool:pay', Config.Prices['dmv:success'])
end
...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('tac_dmv:successschool:addLicense', 'dmv:success')
...
function StartDriveTest(type)
	...
	TriggerServerEvent('tac_dmv:successschool:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		TriggerServerEvent('tac_dmv:successschool:addLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
function Opendmv:successSchoolMenu()
	local ownedLicenses = {}
	...
	if not ownedLicenses['dmv:success'] then
		table.insert(elements, {
			label = (('%s: <span style="color:green;">%s</span>'):format(_U('theory_test'), _U('school_item', ESX.Math.GroupDigits(Config.Prices['dmv:success'])))),
			value = 'theory_test'
		})
	end

	if ownedLicenses['dmv:success'] then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmv:successschool_actions', {
		...
		CurrentAction     = 'dmv:successschool_menu'
...
AddEventHandler('tac_dmv:successschool:hasEnteredMarker', function(zone)
	if zone == 'dmv:successSchool' then
		CurrentAction     = 'dmv:successschool_menu'
		CurrentActionMsg  = _U('press_open_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('tac_dmv:successschool:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('tac_dmv:successschool:loadLicenses')
AddEventHandler('tac_dmv:successschool:loadLicenses', function(licenses)
	Licenses = licenses
end)

-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.dmv:successSchool.Pos.x, Config.Zones.dmv:successSchool.Pos.y, Config.Zones.dmv:successSchool.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_dmv:successschool:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_dmv:successschool:hasExitedMarker', LastZone)
		end
	end
end)
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'dmv:successschool_menu' then
					Opendmv:successSchoolMenu()
				end
