...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('tac_doorlock:getDoorState', function(doorState)
...
AddEventHandler('tac:setJob', function(job) ESX.PlayerData.job = job end)
	...
	while true do
		...
		for k,v in ipairs(Config.DoorList) do
			...
			if v.distanceToPlayer and v.distanceToPlayer < v.maxDistance then
				local size, displayText = 0.6, _U('unlocked')
				...
				if IsControlJustReleased(0, 38) then
					if v.isAuthorized then
						...
						TriggerServerEvent('tac_doorlock:updateState', k, v.locked)
