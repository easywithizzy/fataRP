...
Citizen.CreateThread(function()
    ...
    if Config.Command then
        RegisterCommand(Config.Command, function()
            TriggerEvent('tac_poliisikoira:openMenu')
        end)
    end
    while true do
        local sleep = 250
...
RegisterNetEvent('tac_poliisikoira:openMenu')
AddEventHandler('tac_poliisikoira:openMenu', function()
    mainMenu()
end)

RegisterNetEvent('tac_poliisikoira:hasDrugs')
AddEventHandler('tac_poliisikoira:hasDrugs', function(hadIt)
    if hadIt then
                            ...
                            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
                                ...
                                if distance ~= -1 then
                                    if distance <= 3.0 then
                                        local playerPed = GetPlayerPed(player)
                                        if not IsPedInAnyVehicle(playerPed, true) then
                                            TriggerServerEvent('tac_poliisikoira:hasClosestDrugs', GetPlayerServerId(player))
                                        end
                                    end
                                end
                            end
