...
RegisterNetEvent('tac_holdupweapon:currentlyRobbing')
AddEventHandler('tac_holdupweapon:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('tac_holdupweapon:killBlip')
AddEventHandler('tac_holdupweapon:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('tac_holdupweapon:setBlip')
AddEventHandler('tac_holdupweapon:setBlip', function(position)
...
RegisterNetEvent('tac_holdupweapon:tooFar')
AddEventHandler('tac_holdupweapon:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('tac_holdupweapon:robberyComplete')
AddEventHandler('tac_holdupweapon:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('tac_holdupweapon:startTimer')
AddEventHandler('tac_holdupweapon:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('tac_holdupweapon:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('tac_holdupweapon:tooFar', store)
			end
		end
	end
end)
