...
AddEventHandler('tac_jobs:action', function(job, zone)
	menuIsShowed = true
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			TriggerServerEvent('tac_jobs:startWork', zone.Item)
		end
	...
	elseif zone.Type == "delivery:pay" then
		if Blips['delivery:pay'] ~= nil then
			RemoveBlip(Blips['delivery:pay'])
			Blips['delivery:pay'] = nil
		end
...
function nextStep(gps)
	if gps ~= 0 then
		...
		Blips['delivery:pay'] = AddBlipForCoord(gps.x, gps.y, gps.z)
		SetBlipRoute(Blips['delivery:pay'], true)
			...
			if zones ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					if onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporter" then
						TriggerEvent('tac_jobs:action', job, zone)
					end
				end
				...
				if not isInMarker and hasAlreadyEnteredMarker then
					...
					TriggerEvent('tac_jobs:hasExitedMarker', zone)
				end
			end
		end
	end
