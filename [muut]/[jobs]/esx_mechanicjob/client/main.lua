...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('tac_mechanicjob:onNPCJobCompleted')
	end
end
...
function OpenMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('tac_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TriggerServerEvent('tac_society:removeVehicleFromGarage', 'mechanic', vehicleProps)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('tac_society:openBossMenu', 'mechanic', function(data, menu)
				menu.close()
			end)
		end
	end, function(data, menu)
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				TriggerServerEvent('tac_mechanicjob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TriggerServerEvent('tac_mechanicjob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TriggerServerEvent('tac_mechanicjob:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('tac_mechanicjob:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('tac_mechanicjob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('tac_mechanicjob:startCraft3')
			end
		end, function(data, menu)
			menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
					...
					if NPCOnJob then
						if NPCTargetDeleterZone then
							...
							if CurrentlyTowedVehicle == NPCTargetTowable then
								...
								TriggerServerEvent('tac_mechanicjob:onNPCJobMissionCompleted')
								StopNPCJob()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('tac_mechanicjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('tac_mechanicjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('tac_mechanicjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('tac_mechanicjob:putStockItems', itemName, count)
...
RegisterNetEvent('tac_mechanicjob:onHijack')
AddEventHandler('tac_mechanicjob:onHijack', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('tac_mechanicjob:onCarokit')
AddEventHandler('tac_mechanicjob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('tac_mechanicjob:onFixkit')
AddEventHandler('tac_mechanicjob:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
			Citizen.CreateThread(function()
				exports['progressBars']:startUI(20000, "Korjataan ajoneuvoa")
				Citizen.Wait(20000)
				SetVehicleFixed(vehicle)
				SetVehicleDeformationFixed(vehicle)
				SetVehicleUndriveable(vehicle, false)
...
AddEventHandler('tac_mechanicjob:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_mechanicjob:hasExitedMarker', function(zone)
	if zone =='Vehicledelivery:pay' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('tac_mechanicjob:stopCraft')
		TriggerServerEvent('tac_mechanicjob:stopCraft2')
		TriggerServerEvent('tac_mechanicjob:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('tac_mechanicjob:stopHarvest')
		TriggerServerEvent('tac_mechanicjob:stopHarvest2')
		TriggerServerEvent('tac_mechanicjob:stopHarvest3')
	end
...
AddEventHandler('tac_mechanicjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('tac_mechanicjob:hasExitedEntityZone', function(entity)
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('tac_mechanicjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('tac_mechanicjob:hasExitedMarker', LastZone)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('tac_mechanicjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('tac_mechanicjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('tac_society:putVehicleInGarage', 'mechanic', vehicleProps)
						...
						then
							TriggerServerEvent('tac_service:disableService', 'mechanic')
						end
		...
		if IsControlJustReleased(0, 10) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
