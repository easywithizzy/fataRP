...
AddEventHandler('tac:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('tac_bcbcustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('tac_bcbcustom:installMod')
AddEventHandler('tac_bcbcustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('tac_bcbcustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('tac_bcbcustom:cancelInstallMod')
AddEventHandler('tac_bcbcustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('tac_bcbcustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('tac_bcbcustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('tac_bcbcustom:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'bcb') or not Config.IsbcbJobOnly then
