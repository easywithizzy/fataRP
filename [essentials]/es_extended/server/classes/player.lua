function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.setbank:transferBalance = function(money)
		money = ESX.Math.Round(money)

		if money >= 0 then
			self.player.setbank:transferBalance(money)
		else
			print(('es_extended: %s attempted exploiting! (reason: player tried setting -1 bank:transfer balance)'):format(self.identifier))
		end
	end

	self.getbank:transfer = function()
		return self.player.get('bank:transfer')
	end
	...
	self.addbank:transfer = function(money)
		money = ESX.Math.Round(money)

		if money >= 0 then
			self.player.addbank:transfer(money)
		else
			print(('es_extended: %s attempted exploiting! (reason: player tried adding -1 bank:transfer balance)'):format(self.identifier))
		end
	end

	self.removebank:transfer = function(money)
		money = ESX.Math.Round(money)

		if money >= 0 then
			self.player.removebank:transfer(money)
		else
			print(('es_extended: %s attempted exploiting! (reason: player tried removing -1 bank:transfer balance)'):format(self.identifier))
		end
	end
	...
	self.displaybank:transfer = function(money)
		self.player.displaybank:transfer(money)
	end
	...
	self.getAccounts = function()
		...
		for k,v in ipairs(Config.Accounts) do
			if v == 'bank:transfer' then
				table.insert(accounts, {
					name  = 'bank:transfer',
					money = self.get('bank:transfer'),
					label = Config.AccountLabels.bank:transfer
				})
			else
	...
	self.setAccountMoney = function(acc, money)
		...
		if acc == 'bank:transfer' then
			self.set('bank:transfer', newMoney)
		end
