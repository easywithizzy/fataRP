		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				TriggerClientEvent('tac:showNotification', target,  _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name))
                                ...
                                TriggerEvent("tac:giveitemalert",sourceXPlayer.name,targetXPlayer.name,ESX.Items[itemName].label,itemCount)
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerClientEvent('tac:showNotification', target,  _U('received_money', ESX.Math.GroupDigits(itemCount), sourceXPlayer.name))
                        ...
                        TriggerEvent("tac:givemoneyalert",sourceXPlayer.name,targetXPlayer.name,itemCount)
		else
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('tac:showNotification', target,  _U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName], sourceXPlayer.name))
                        ...
                        TriggerEvent("tac:givemoneybank:transferalert",sourceXPlayer.name,targetXPlayer.name,itemCount)
		else
			TriggerClientEvent('tac:showNotification', _source, _U('imp_invalid_amount'))
		end
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				...
				TriggerClientEvent('tac:showNotification', target,  _U('received_weapon', weaponLabel, sourceXPlayer.name))
                                ...
                                TriggerEvent("tac:giveweaponalert",sourceXPlayer.name,targetXPlayer.name,weaponLabel)
			end
		else
			TriggerClientEvent('tac:showNotification', _source, _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel))
			TriggerClientEvent('tac:showNotification', target, _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel))
		end
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local pickupLabel = ('~w~%s~s~ [~r~%s~s~]'):format(xItem.label, itemCount)
	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				local pickupLabel = ('~w~%s~s~ [~g~%s~s~]'):format(_U('cash'), _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('~w~%s~s~ [~g~%s~s~]'):format(account.label, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
