...
ESX.ShowInventory = function()
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 then
			...
			local canDrop = ESX.PlayerData.accounts[i].name ~= 'bank:transfer'
			...
			if data1.current.action == 'give' then
				...
				for i=1, #players, 1 do
					if players[i] ~= PlayerId() then
						...
						table.insert(elements, {
							label = "Pelaaja: "..pelaajat,
							player = players[i]
						})
						
						local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(players[i]))
						ESX.ShowAdvancedNotification('Pelaaja', pelaajat, "", mugshotStr, 1)
