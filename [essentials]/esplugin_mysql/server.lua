...
AddEventHandler('es_db:createUser', function(identifier, license, cash, bank:transfer, callback)
	local user = {
		identifier = identifier,
		money = cash or 0,
		bank:transfer = bank:transfer or 0,
		license = license,
		group = 'user',
		permission_level = 0
	}

	MySQL.Async.execute('INSERT INTO users (`identifier`, `money`, `bank:transfer`, `group`, `permission_level`, `license`) VALUES (@identifier, @money, @bank:transfer, @group, @permission_level, @license);',
	{
		identifier = user.identifier,
		money = user.money,
		bank:transfer = user.bank:transfer,
