...
-- OOP hack
function CreatePlayer(source, permission_level, money, bank:transfer, identifier, license, group, roles)
	local self = {}
	...
	self.money = money
	self.bank:transfer = bank:transfer
	self.identifier = identifier
	self.license = license
	self.group = group
	self.coords = {x = 0.0, y = 0.0, z = 0.0}
	self.session = {}
	self.bank:transferDisplayed = false
	...
	-- Sets a players bank:transfer balance
	rTable.setbank:transferBalance = function(m)
		if type(m) == "number" then
			-- Triggers an event to save it to the database
			TriggerEvent("es:setPlayerData", self.source, "bank:transfer", m, function(response, success)
				self.bank:transfer = m
			end)
		else
			log('ES_ERROR: There seems to be an issue while setting bank:transfer, something else then a number was entered.')
			print('ES_ERROR: There seems to be an issue while setting bank:transfer, something else then a number was entered.')
		end
	end

	-- Returns the players bank:transfer
	rTable.getbank:transfer = function()
	...
	-- Adds money to a users bank:transfer
	rTable.addbank:transfer = function(m)
		if type(m) == "number" then
			local newbank:transfer = self.bank:transfer + m
			self.bank:transfer = newbank:transfer

			-- Triggers an event to tell the UI components money was just added
			TriggerClientEvent("es:addedbank:transfer", self.source, m)
		else
			log('ES_ERROR: There seems to be an issue while adding to bank:transfer, a different type then number was trying to be added.')
			print('ES_ERROR: There seems to be an issue while adding to bank:transfer, a different type then number was trying to be added.')
		end
	end

	-- Removes money from a users bank:transfer
	rTable.removebank:transfer = function(m)
		if type(m) == "number" then
			local newbank:transfer = self.bank:transfer - m
			self.bank:transfer = newbank:transfer

			-- Triggers an event to tell the UI components money was just removed
			TriggerClientEvent("es:removedbank:transfer", self.source, m)
		else
			log('ES_ERROR: There seems to be an issue while removing from bank:transfer, a different type then number was trying to be removed.')
			print('ES_ERROR: There seems to be an issue while removing from bank:transfer, a different type then number was trying to be removed.')
		end
	end
	...
	-- Used to initially display someones bank:transfer
	rTable.displaybank:transfer = function(m)
		if type(m) == "number" then
			if not self.bank:transferDisplayed then
				-- Triggers an event to tell the UI components to start displaying bank:transfer
				TriggerClientEvent("es:displaybank:transfer", self.source, math.floor(m))
				self.bank:transferDisplayed = true
			end
		else
			log('ES_ERROR: There seems to be an issue while displaying bank:transfer, a different type then number was trying to be shown.')
			print('ES_ERROR: There seems to be an issue while displaying bank:transfer, a different type then number was trying to be shown.')
		end
	end
