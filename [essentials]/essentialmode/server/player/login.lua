...
function LoadUser(identifier, source, new, licenseNotRequired)
	...
	db.retrieveUser(identifier, function(user, isJson)
		...
		if user.license or licenseNotRequired then
			...
			Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank:transfer, user.identifier, user.license, user.group, user.roles or "")
	...
	SetTimeout(60000, function()
		Citizen.CreateThread(function()
			for k,v in pairs(Users)do
				if Users[k] ~= nil then
					db.updateUser(v.get('identifier'), {money = v.getMoney(), bank:transfer = v.getbank:transfer()}, function()end)
				end
			end
