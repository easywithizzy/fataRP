...
MySQL.ready(function()
	...
	MySQL.Async.fetchAll('SELECT * FROM weashops', {}, function(result)
		...
		TriggerClientEvent('tac_weaponshop:sendShop', -1, shopItems)
	end)

end)

ESX.RegisterServerCallback('tac_weaponshop:getShop', function(source, cb)
	cb(shopItems)
end)

ESX.RegisterServerCallback('tac_weaponshop:buyLicense', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.getMoney() >= Config.LicensePrice then
		xPlayer.removeMoney(Config.LicensePrice)

		TriggerEvent('tac_license:addLicense', source, 'weapon', function()
			cb(true)
		end)
	else
...
ESX.RegisterServerCallback('tac_weaponshop:buyWeapon', function(source, cb, weaponName, zone)
	local xPlayer = ESX.GetPlayerFromId(source)
