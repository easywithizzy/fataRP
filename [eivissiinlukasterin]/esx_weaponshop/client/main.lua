...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('tac_weaponshop:getShop', function(shopItems)
...
RegisterNetEvent('tac_weaponshop:sendShop')
AddEventHandler('tac_weaponshop:sendShop', function(shopItems)
	for k,v in pairs(shopItems) do
		Config.Zones[k].Items = v
	end
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('tac_weaponshop:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu(zone)
				end
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.TriggerServerCallback('tac_weaponshop:buyWeapon', function(bought)
			if bought then
...
AddEventHandler('tac_weaponshop:hasEnteredMarker', function(zone)
	if zone == 'pistolShop' or zone == 'smgShop' or zone == 'arShop'  then
		CurrentAction     = 'shop_menu'
		CurrentActionMsg  = _U('shop_menu_prompt')
		CurrentActionData = { zone = zone }
	end
...
AddEventHandler('tac_weaponshop:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('tac_weaponshop:hasEnteredMarker', currentZone)
		end
		
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_weaponshop:hasExitedMarker', LastZone)
		end
	end
end)
			...
			if IsControlJustReleased(0, 38) then
				...
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable and Config.Zones[CurrentActionData.zone].Legal then
						ESX.TriggerServerCallback('tac_license:checkLicense', function(hasWeaponLicense)
