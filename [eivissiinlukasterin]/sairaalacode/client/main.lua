...
AddEventHandler('mythic_hospital:client:RPSendToBed', function(id, data)
    ...
    Citizen.CreateThread(function()
        while bedOccupyingData ~= nil do
            Citizen.Wait(1)
            PrintHelpText('Paina ~INPUT_VEH_DUCK~ noustaksesi ylös')
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        --exports['NoPixel_notify']:DoHudText('inform', 'Doctors Are Treating You')
        ESX.ShowNotification('Lääkäri hoitaa sinua')
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_ambulance', 'Sairaalamaksu', bill)
    print(bill)
    --exports['mythic_notify']:SendAlert('inform', 'You\'ve been treated and billed.')
    ESX.ShowNotification('Olet vastaanottanut laskun hoidosta. Muista maksaa se pikaisesti.')
    LeaveBed()
end)
        ...
        if distance < 10 then
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 3 then
					...
					ESX.Game.Utils.DrawText3D(vector3(hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z + 0.5), '[E] Kirjaudu sisään', 0.4)
					if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                            exports['mythic_progbar']:Progress({
                                name = "hospital_action",
                                duration = 10500,
                                label = "Tarkistetaan tietoja...",
                                useWhileDead = true,
                                canCancel = true,
                                controlDisables = {
                            ...
                            ESX.ShowNotification('Et tarvitse lääkinnällistä hoitoa.')
                        end
                    end
                end
            end
