...
AddEventHandler('outlawNotify', function(alert)
		if PlayerData.job ~= nil and PlayerData.job.name == 'police' == 'sheriff' then
            Notify(alert)
        end
end)
...
Citizen.CreateThread( function()
    while true do
        ...
        if pedIsTryingToSellDrugs then
			...
			if PlayerData.job ~= nil and PlayerData.job.name == 'police' == 'sheriff' and showcopsmisbehave == false then
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'police' == 'sheriff' and showcopsmisbehave then
				ESX.TriggerServerCallback('tac_skin:getPlayerSkin', function(skin, jobSkin)
					local sex = nil
					if skin.sex == 0 then
						sex = "mies" --male/change it to your language
					else
						sex = "nainen" --female/change it to your language
					end
...
AddEventHandler('drugsPlace', function(tx, ty, tz)
	if PlayerData.job.name == 'police' == 'sheriff' then
		local transT = 250
