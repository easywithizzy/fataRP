...
Citizen.CreateThread(function()
	...
	if jengissa then
		TriggerServerEvent('tac_jengialueet:fetchmestat')
	end
end)


RegisterNetEvent('tac_jengialueet:mestat')
AddEventHandler('tac_jengialueet:mestat', function(result)
	if jengissa then
		hahaatable = result
		if blipit then
			for i = 1, #Areas do
...
AddEventHandler('tac:setJob', function()
	jengissa = false
	PlayerData = ESX.GetPlayerData()
	for i = 1, #Jobit, 1 do
		if PlayerData.job.name == Jobit[i] then
...
RegisterNetEvent('tac_jengialueet:currentlyclaiming')
AddEventHandler('tac_jengialueet:currentlyclaiming', function(mesta)
	alueenperkele = mesta
	holdingup = true
end)

RegisterNetEvent('tac_jengialueet:menetetty')
AddEventHandler('tac_jengialueet:menetetty', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,4)
end)

RegisterNetEvent('tac_jengialueet:killblip')
AddEventHandler('tac_jengialueet:killblip', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,69)
	ESX.ShowNotification('~g~Aluettasi ei saatu vallattua!!!')
end)

RegisterNetEvent('tac_jengialueet:setblip')
AddEventHandler('tac_jengialueet:setblip', function(kohte)
	SetBlipColour(Areas[kohte].nameofarea,59)
	ESX.ShowNotification("~r~Aluettasi Vallataan!!!")
end)

RegisterNetEvent('tac_jengialueet:toofarlocal')
AddEventHandler('tac_jengialueet:toofarlocal', function()
	holdingup = false
	claimingName = ""
end)

RegisterNetEvent('tac_jengialueet:muille')
AddEventHandler('tac_jengialueet:muille', function(alue)
	ESX.ShowNotification('Jengisi on vallannut alueen')
	SetBlipColour(Areas[alue].nameofarea,69)
end)

RegisterNetEvent('tac_jengialueet:claimcomplete')
AddEventHandler('tac_jengialueet:claimcomplete', function()
	holdingup = false
	alueenperkele = ""
end)

RegisterNetEvent('tac_jengialueet:valloitusilmoitus') --testi 28.5
AddEventHandler('tac_jengialueet:valloitusilmoitus', function(notifikaatio)
...
RegisterNetEvent('tac_jengialueet:starttimer')
AddEventHandler('tac_jengialueet:starttimer', function()
	...
	while true do
		...
		if not holdingup then
					...
					if IsControlJustReleased(0, 38) then -- käynnistä ryöstö
						...
						if IsPedArmed(PlayerPedId(), 7) then
							if not jengissa then-- onko asetta								
								...
								TriggerServerEvent('tac_jengialueet:claim', area)
							end
						else
			...
			if Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > Config.MaxDistance then
				TriggerServerEvent('tac_jengialueet:toofar', alueenperkele)
				timer = 0
			end
			...
			if laskuri == Config.Montakoalistetaan then
				TriggerServerEvent('tac_jengialueet:rostoohi', alueenperkele)
				holdingup = false
				laskuri = 0
			end
