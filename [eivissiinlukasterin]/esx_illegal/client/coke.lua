...
function ProcessCoke()
	...
	TriggerServerEvent('tac_illegal:processCocaLeaf')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CokeProcessing.coords, false) > 5 then
			...
			TriggerServerEvent('tac_illegal:cancelProcessing')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('tac_illegal:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('tac_illegal:pickedUpCocaLeaf')
					else
