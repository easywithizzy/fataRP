...
function checkPluginVersion() {
	if (isPluginVersionCorrect()) {
		document.getElementById('pluginVersion').innerHTML = `Plugin versio: <font color="green">${voip.pluginVersion}</font> (up-to-date)`;
	} else {
		document.getElementById('pluginVersion').innerHTML = `Plugin versio: <font color="red">${voip.pluginVersion}</font> (Required: ${voip.minVersion})`;
...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Connected to the wrong TeamSpeak server, please join the server: <font color="#00B0FF">${voip.plugin_data.TSServer}</font>`;
			screenMessage = 'Wrong TeamSpeak server';
			color = 'blue';
			break;
		case WRONG_CHANNEL:
			msg = `Connected to the wrong TeamSpeak channel, please join the channel: <font color="#00B0FF">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[Puhe]: ${msg}</font>`;
	}
	document.getElementById('pluginStatus').innerHTML = `Plugin tilanne: <font color="${color}">${screenMessage || msg}</font>`;
}

function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `TeamSpeak serveri: <font color="#00B0FF">${voip.plugin_data.TSServer}</font>`;
	document.getElementById('TSChannel').innerHTML = `TeamSpeak kanava: <font color="#00B0FF">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
